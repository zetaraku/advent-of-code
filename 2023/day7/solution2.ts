...
function typeOfHand(cards: Card[]): HandType {
  const nonJokerCards = cards.filter((card) => card.label !== "J");

  const distribution = Object.entries(
    Object.groupBy(nonJokerCards, (card) => card.label),
  )
  ...
  // 1 JOKER
  if (listEqual(distribution, [4])) return HandType.FIVE_OF_A_KIND;
  if (listEqual(distribution, [3, 1])) return HandType.FOUR_OF_A_KIND;
  if (listEqual(distribution, [2, 2])) return HandType.FULL_HOUSE;
  if (listEqual(distribution, [2, 1, 1])) return HandType.THREE_OF_A_KIND;
  if (listEqual(distribution, [1, 1, 1, 1])) return HandType.ONE_PAIR;

  // 2 JOKER
  if (listEqual(distribution, [3])) return HandType.FIVE_OF_A_KIND;
  if (listEqual(distribution, [2, 1])) return HandType.FOUR_OF_A_KIND;
  if (listEqual(distribution, [1, 1, 1])) return HandType.THREE_OF_A_KIND;

  // 3 JOKER
  if (listEqual(distribution, [2])) return HandType.FIVE_OF_A_KIND;
  if (listEqual(distribution, [1, 1])) return HandType.FOUR_OF_A_KIND;

  // 4 JOKER
  if (listEqual(distribution, [1])) return HandType.FIVE_OF_A_KIND;

  // 5 JOKER
  if (listEqual(distribution, [])) return HandType.FIVE_OF_A_KIND;
